...
namespace TheTechIdea.Beep.Caching
{
    ...
    /// </summary>
    [AddinAttribute(Category = DatasourceCategory.INMEMORY, DatasourceType = DataSourceType.CachedMemory)]
    public class CachedMemoryDataSource : IDataSource
    {
        ...
        public DataSourceType DatasourceType { get; set; } = DataSourceType.CachedMemory;
        ...
        public CachedMemoryDataSource(string datasourcename, IDMLogger logger, IDMEEditor pDMEEditor, DataSourceType pDatasourceType, IErrorsInfo per)
            : this()
        {
            ...
            // Initialize with MemoryCache provider specifically
        ...
        private void InitializeCacheProvider()
        {
            try
            {
                // Create a dedicated MemoryCache provider for this data source
                var config = new CacheConfiguration
                {
                    DefaultExpiry = TimeSpan.FromHours(12), // Shorter expiry for cached memory
                    MaxItems = 25000, // Moderate limit for memory management
                    EnableStatistics = true,
                    KeyPrefix = $"cached:{DatasourceName}:",
                    MemoryCache = new MemoryCacheConfiguration
                    {
                        SizeLimit = 50 * 1024 * 1024, // 50MB limit
                        CompactionPercentage = 0.10, // 10% compaction
                        ExpirationScanFrequency = TimeSpan.FromMinutes(2)
                    }
                };

                _cacheProvider = new Providers.MemoryCacheProvider(config);
                Logger?.WriteLog($"MemoryCache provider initialized for '{DatasourceName}'.");
            }
            catch (Exception ex)
            {
                Logger?.WriteLog($"Error initializing MemoryCache provider: {ex.Message}");
                // Create a fallback simple provider
        ...
        private void InitializeConnection()
        {
            ...
            {
                // Create default connection properties for cached memory
                Dataconnection.ConnectionProp = new ConnectionProperties
                {
                    ConnectionName = DatasourceName,
                    DatabaseType = DataSourceType.CachedMemory,
                    Category = DatasourceCategory.INMEMORY,
                    DriverName = "CachedMemoryDataSource",
                    DriverVersion = "1.0.0"
                };
            }
        }
        ...
        public ConnectionState Openconnection()
        {
            try
            {
                ...
                Logger?.WriteLog($"Cached Memory Data Source '{DatasourceName}' opened successfully.");
            }
            catch (Exception ex)
            {
                ...
                Logger?.WriteLog($"Error opening Cached Memory Data Source: {ex.Message}");
            }
        ...
        public ConnectionState Closeconnection()
        {
            ...
            Logger?.WriteLog($"Cached Memory Data Source '{DatasourceName}' closed.");
            return ConnectionStatus;
        }
        ...
        public IErrorsInfo InsertEntity(string EntityName, object InsertedData)
        {
            ...
            try
            {
                ...
                // Also store in MemoryCache provider with expiration policy
                if (_cacheProvider != null)
                {
                    var cacheKey = $"{EntityName}:{key}";
                    _ = _cacheProvider.SetAsync(cacheKey, dataDict, TimeSpan.FromHours(12));
                }

                _entityTimestamps[EntityName] = DateTime.UtcNow;
                Logger?.WriteLog($"Entity '{EntityName}' record inserted with key '{key}' in Cached Memory.");
            }
            catch (Exception ex)
            {
                ErrorObject.Flag = Errors.Failed;
        ...
        public IErrorsInfo UpdateEntity(string EntityName, object UploadDataRow)
        {
            ...
            try
            {
                ...
                if (entityCache.ContainsKey(key))
                {
                    ...
                    // Update in MemoryCache provider
                    if (_cacheProvider != null)
                    {
                        var cacheKey = $"{EntityName}:{key}";
                        _ = _cacheProvider.SetAsync(cacheKey, dataDict, TimeSpan.FromHours(12));
                    ...
                    _entityTimestamps[EntityName] = DateTime.UtcNow;
                    Logger?.WriteLog($"Entity '{EntityName}' record updated with key '{key}' in Cached Memory.");
                }
                else
                {
                    ErrorObject.Flag = Errors.Failed;
        ...
        public IErrorsInfo DeleteEntity(string EntityName, object UploadDataRow)
        {
            ...
            try
            {
                ...
                {
                    // Remove from MemoryCache provider
                    if (_cacheProvider != null)
                    {
                        var cacheKey = $"{EntityName}:{key}";
                    ...
                    _entityTimestamps[EntityName] = DateTime.UtcNow;
                    Logger?.WriteLog($"Entity '{EntityName}' record deleted with key '{key}' from Cached Memory.");
                }
                else
                {
                    ErrorObject.Flag = Errors.Failed;
        ...
        {
            ErrorObject.Flag = Errors.Failed;
            ErrorObject.Message = "Transactions not supported in Cached Memory Data Source.";
            return ErrorObject;
        }
        ...
        {
            ErrorObject.Flag = Errors.Failed;
            ErrorObject.Message = "SQL execution not supported in Cached Memory Data Source.";
            return ErrorObject;
        }
        ...
        {
            ErrorObject.Flag = Errors.Failed;
            ErrorObject.Message = "Script execution not supported in Cached Memory Data Source.";
            return ErrorObject;
        }
        ...
        {
            ErrorObject.Flag = Errors.Failed;
            ErrorObject.Message = "Bulk update not supported in Cached Memory Data Source.";
            return ErrorObject;
        }
        #endregion
