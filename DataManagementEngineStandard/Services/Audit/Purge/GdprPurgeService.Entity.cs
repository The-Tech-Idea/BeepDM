...
{
    /// <summary>
    /// Entity-scoped purge half of <see cref="GdprPurgeService"/>.
    /// </summary>
    public sealed partial class GdprPurgeService
    {
        /// <summary>
        /// Removes every audit event whose
        /// <see cref="AuditEvent.EntityName"/> +
        /// <see cref="AuditEvent.RecordKey"/> matches the supplied tuple
        /// and re-seals every affected chain.
        /// </summary>
        public async Task<int> PurgeByEntityAsync(
            string entityName,
            string recordKey,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (string.IsNullOrEmpty(entityName) || string.IsNullOrEmpty(recordKey))
            {
                return 0;
            }
            ...
            foreach (IAuditPurgeStore store in _stores)
            {
                ...
                PurgeImpact impact = await store.DeleteByEntityAsync(entityName, recordKey, cancellationToken)
            ...
            await EmitPurgeAuditAsync(
                operation: "PurgeByEntity",
                target: entityName + "/" + recordKey,
