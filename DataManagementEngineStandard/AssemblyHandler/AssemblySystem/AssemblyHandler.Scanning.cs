                ...
                if (types.Length > 100)
                {
                    ...
                    Parallel.ForEach(types, type =>
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Logger?.WriteLog($"ScanAssembly: Failed to read type info in {asm?.GetName()?.Name}: {ex.Message}");
                        }
                    });
                ...
                else
                {
                    ...
                    foreach (var type in types)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Logger?.WriteLog($"ScanAssembly: Failed to process type in {asm?.GetName()?.Name}: {ex.Message}");
                        }
                    }
                }
        ...
        private void ProcessTypeInfo(TypeInfo typeInfo, Assembly asm)
        {
            try
            {
                ...
                if (typeInfo.ImplementedInterfaces.Contains(typeof(ISchemaMigrationProvider)))
                {
                    ...
                    if (attr != null)
                    {
                        ConfigEditor?.MigrationProviders?.Register(attr.DataSourceType,
                            _ => (ISchemaMigrationProvider)Activator.CreateInstance(providerType, _));
                        ConfigEditor?.MigrationProviders?.RegisterCategoryFallback(attr.Category,
                            _ => (ISchemaMigrationProvider)Activator.CreateInstance(providerType, _));
                    }
                }
