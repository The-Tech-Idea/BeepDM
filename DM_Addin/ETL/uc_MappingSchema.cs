...
namespace TheTechIdea.ETL
{
    public partial class uc_MappingSchema : UserControl, IDM_Addin
    {
       ...
       // public event EventHandler<PassedArgs> OnObjectSelected;
        ...
        private void FindSimiliarEntityinDestRow()
        {

            Mapping_rep m = (Mapping_rep)mapsBindingSource.Current;
            ds2 = DMEEditor.GetDataSource(m.Entity2DataSource);
            Dictionary<string, double> pairs = new Dictionary<string, double>();
            if (ds2 != null)
            {
                ds2.GetEntitesList();
                if (ds2.EntitiesNames.Count > 0)
                {
                    foreach (string str in ds2.EntitiesNames)
                    {
                        double u=DMEEditor.Utilfunction.CalculateSimilarity(m.EntityName1, str);
                        pairs.Add(str, u);
                    }
                    m.EntityName2 = pairs.Where(u=>u.Value==((pairs.Min(p => p.Value)))).FirstOrDefault().Key;
                }

            }
        }
        private void FindSimiliarEntityinDestAll()
        {
                ...
                if (ds2.EntitiesNames.Count > 0)
                {
                    ...
                    for (int i = 0; i < mapsBindingSource.Count; i++)
                    {
                        ...
                        foreach (string str in ds2.EntitiesNames)
                        {
                            ...
                            found= DMEEditor.Utilfunction.KMPSearch(m.EntityName1.ToLower(), str.ToLower());   //CalculateSimilarity(m.EntityName1.ToLower(), str.ToLower());
                            if (found)
                            {
                                m.EntityName2 = str;
                                break;
                         ...
                         //pairs.Where(u => u.Value == ((pairs.Max(p => p.Value)))).FirstOrDefault().Key;
                        mapsBindingSource.MoveNext();
                    }
