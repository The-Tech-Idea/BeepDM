...
namespace TheTechIdea.Tools
{
    ...
    public class AssemblyHandler : IAssemblyHandler
    {
        ...
        public void CheckDriverAlreadyExistinList()
        {
            ...
            {
                ConnectionDriversConfig founddr = DMEEditor.ConfigEditor.DataDrivers.Where(c => c.DriverClass == dr.DriverClass && c.version == dr.version).FirstOrDefault();
                if (founddr == null)
                {
        ...
        private ConnectionDriversConfig GetNonADODrivers(Assembly asm)
        {
            ...
            try
            {
                ...
                string[] p = asm.FullName.Split(new char[] { ',' });
                p[1] = p[1].Substring(p[1].IndexOf("=") + 1);
                ConnectionDriversConfig found = DMEEditor.ConfigEditor.DriverDefinitions.Where(o => o.PackageName == p[0]).FirstOrDefault();
                if (found != null)
                {
                    ...
                    driversConfig.DriverClass = p[0];
                    driversConfig.dllname = asm.ManifestModule.Name;
                    driversConfig.parameter1 = p[0] + "." + found.parameter1;
                    driversConfig.parameter2 = p[0] + "." + found.parameter2;
                    driversConfig.parameter3 = p[0] + "." + found.parameter3;
                    driverfound = true;
                    DataDrivers.Add(driversConfig);
                }
